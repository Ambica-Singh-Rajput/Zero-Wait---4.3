...
const WelcomeScreen: React.FC<WelcomeScreenProps> = ({ onNext }) => {
  ...
  const handleEmergencyClick = async () => {
    ...
    try {
      ...
      // Use existing location or fallback immediately
      const location = state.userLocation || { lat: 28.7041, lng: 77.1025 }
      if (!state.userLocation) {
        dispatch({ type: 'SET_LOCATION', payload: location })
      }
      
      // Start ambulance booking immediately (fast mode)
      const bookingPromise = bookAmbulance(
        location,
        'Nearest Emergency Hospital',
        'Emergency Location',
        'Emergency - need immediate help',
        'Unknown condition - requires assessment',
        true // Emergency mode - no delays
      )
      
      // Run non-critical operations in parallel
      const parallelOps = Promise.all([
        // Play sound (non-blocking)
        Promise.resolve(playEmergencyAlert()),
        // Request notifications (non-blocking, don't wait)
        requestNotificationPermission().catch(() => {}),
        // Try to get better location (non-blocking)
        state.userLocation ? Promise.resolve() : 
          requestEmergencyLocation().then(pos => {
            const betterLocation = { lat: pos.coords.latitude, lng: pos.coords.longitude }
            dispatch({ type: 'SET_LOCATION', payload: betterLocation })
          }).catch(() => {})
      ])
      
      // Wait for ambulance booking (the critical path)
      ...
      showBookingConfirmation(booking.ambulanceId, `${booking.estimatedArrival} minutes`)
      ...
      // Even on error, proceed to ambulance page
      dispatch({ 
        type: 'SET_SITUATION', 
        ...
        <div className="relative">
          ...
          >
            <div className="flex items-center justify-center space-x-3 mb-2">
              {isBookingEmergency ? (
                <>
                  ...
                  <span className="text-xl">?? DISPATCHING...</span>
                </>
              ) : (
                <>
                  <div className="relative">
            ...
            <div className="text-sm font-medium opacity-95 bg-black/20 rounded-lg px-3 py-2">
              {isBookingEmergency 
                ? '?? Dispatching ambulance in 2 seconds...' 
