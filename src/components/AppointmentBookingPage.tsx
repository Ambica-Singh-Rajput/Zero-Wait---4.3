  ...
  const confirmBooking = async () => {
    ...
    try {
      const newAppointment = {
        id: appointmentId,
        hospitalName: state.selectedHospital?.name || 'City General Hospital',
        doctorName: selectedSlot?.doctor || 'Unknown Doctor',
        department: selectedSlot?.department || 'General',
        date: selectedSlot?.date || 'Today',
        time: selectedSlot?.time || 'Pending',
        status: 'upcoming',
        type: state.situation === 'emergency' ? 'emergency' : 'normal',
        symptoms: state.userInput || 'Not specified',
        cost: selectedSlot?.cost || 500,
        address: state.selectedHospital?.address || 'Hospital Location',
        phone: state.selectedHospital?.phone || 'Contact Hospital'
      }
      
      const existing = JSON.parse(localStorage.getItem('user_appointments') || '[]')
      localStorage.setItem('user_appointments', JSON.stringify([...existing, newAppointment]))
    } catch (e) {
    ...
    const confirmMessage: ChatMessage = {
      ...
      type: 'bot',
      message: `Great! Your appointment has been confirmed for ${selectedSlot?.time} with ${selectedSlot?.doctor} in ${selectedSlot?.department}. Appointment ID: ${appointmentId}`,
      timestamp: new Date()
    }
