...
const AppointmentsDashboard: React.FC = () => {
  ...
  // Mock appointments data + localStorage
  useEffect(() => {
    ...
    try {
      const stored = localStorage.getItem('user_appointments')
      if (stored) {
        const userAppointments = JSON.parse(stored)
        // Add user appointments at the top
        setAppointments([...userAppointments, ...mockAppointments])
        return
      }
    } catch (e) {
