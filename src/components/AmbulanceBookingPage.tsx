...
const AmbulanceBookingPage: React.FC = () => {
  ...
  useEffect(() => {
    if (!state.ambulanceBooking) {
      ...
      // Ensure instructions are loaded even if booking was passed from previous screen
      const instructions = getPatientCareInstructions(state.ambulanceBooking.emergencyType)
      setPatientInstructions(instructions)
    }
