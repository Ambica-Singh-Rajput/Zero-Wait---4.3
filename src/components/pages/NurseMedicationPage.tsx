...
const NurseMedicationPage: React.FC = () => {
  ...
  const overdueCount = medicationSchedule.filter(m => m.urgency === 'OVERDUE').length
  const dueSoonCount = medicationSchedule.filter(m => m.urgency === 'DUE SOON').length
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8 grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-emerald-600">Total Scheduled</p>
                <p className="text-3xl font-bold text-emerald-700">{medicationSchedule.length}</p>
              </div>
          ...
          <div className="mb-6">
            <div className="bg-red-50 border border-red-200 rounded-lg p-4">
              ...
              <div className="space-y-2">
                {medicationSchedule.filter(m => m.urgency === 'OVERDUE').map((med) => (
                  ...
                  {patient.currentMedications.map((med) => (
                    <div key={med.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                      <div className="flex-1">
                        ...
                        <div className="text-xs text-gray-500">
                          Route: {med.route} | Prescribed by: {med.prescribedBy}
                        </div>
                      </div>
                      <span className={`px-2 py-1 rounded text-xs font-medium ${
                        ...
                        {med.administered ? 'Given' : 'Pending'}
                      </span>
                    </div>
                  ))}
                </div>
