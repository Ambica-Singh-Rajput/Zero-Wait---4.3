...
import { ArrowLeft, Activity, AlertCircle, TrendingUp, Heart, Thermometer, Droplets, Wind, CheckCircle, X, AlertTriangle } from 'lucide-react'
...
const NurseVitalSignsPage: React.FC = () => {
  ...
  const [alertPrecautions, setAlertPrecautions] = useState<string[]>([])
  ...
  const handleViewDetails = (patient: NursePatient) => {
    console.log('View Details clicked for:', patient.name)
    setSelectedPatient(patient)
    setShowDetailsModal(true)
  }

  const handleSetAlert = async (patient: NursePatient) => {
    ...
    // Only allow alerts for critical or warning patients
    if (patient.vitalSigns[0].status === 'normal') {
      setAlertMessage(`${patient.name} is currently stable. Alerts are only available for patients with critical or warning status.`)
      setAlertPrecautions([])
    ...
    try {
      ...
      const alert = await alertService.createPatientAlert(
        patient.id,
        patient.name,
        patient.room,
        patient.vitalSigns[0].status as 'critical' | 'warning',
        patient.vitalSigns[0]
      )

      setAlertMessage(`ALERT SET: ${patient.name} in Room ${patient.room} - ${patient.vitalSigns[0].status.toUpperCase()} status detected. All nursing staff have been notified.`)
      setAlertPrecautions(alert.precautions)
      setShowAlertModal(true)
      
      console.log('Alert created successfully:', alert)
    } catch (error) {
      console.error('Error creating alert:', error)
      setAlertMessage(`Error setting alert for ${patient.name}. Please try again.`)
      setAlertPrecautions([])
      setShowAlertModal(true)
    }
  }
  ...
  const handleExportData = (patient: NursePatient) => {
    console.log('Export Data clicked for:', patient.name)
    setSelectedPatient(patient)
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-red-50 border border-red-200 rounded-xl p-6">
            ...
            {criticalPatients.length > 0 && (
              <button className="mt-3 text-xs text-red-700 font-medium hover:text-red-800">
                Set Alerts for Critical Patients
              </button>
            )}
          </div>
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-6">
            ...
            {warningPatients.length > 0 && (
              <button className="mt-3 text-xs text-yellow-700 font-medium hover:text-yellow-800">
                Set Alerts for Warning Patients
              </button>
            )}
          </div>
        ...
        {criticalPatients.length > 0 && (
          <div className="bg-red-100 border border-red-300 rounded-xl p-4 mb-6">
            ...
            <p className="text-red-700 mt-1">
              {criticalPatients.length} patient(s) require immediate attention. Click "Set Alert" to notify all nursing staff.
            </p>
          </div>
        )}
              ...
              <div className="p-6">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg hover:opacity-90 flex items-center text-sm ${
                      patient.vitalSigns[0].status === 'normal' 
                        ? 'bg-gray-400 text-gray-200 cursor-not-allowed'
                        : 'bg-orange-600 text-white hover:bg-orange-700'
                    }`}
                    disabled={patient.vitalSigns[0].status === 'normal'}
                  >
                    <AlertCircle size={16} className="mr-2" />
                    Set Alert
                    {patient.vitalSigns[0].status !== 'normal' && (
                      <span className="ml-1 px-2 py-0.5 bg-white bg-opacity-20 rounded text-xs">
                        AI
                      </span>
                    )}
                  </button>
                  ...
                  <div className="mt-4 p-3 bg-orange-50 border border-orange-200 rounded-lg">
                    <div className="flex items-center space-x-2">
                      ...
                      <p className="text-sm text-orange-800">
                        {patient.vitalSigns[0].status === 'critical' 
                          ? 'Critical status detected - Immediate attention required'
                          : 'Warning status detected - Close monitoring recommended'}
                      </p>
                    </div>
                  </div>
                )}
