...
export const bookAmbulance = async (
  ...
  // Only add delay for non-emergency bookings
  if (!isEmergency) {
  ...
  // Calculate estimated arrival time (generate a realistic small number for demo purposes)
  // Since real user location might be thousands of miles from the mock ambulances
  const estimatedArrival = Math.floor(Math.random() * 4) + 5 // 5 to 8 minutes
